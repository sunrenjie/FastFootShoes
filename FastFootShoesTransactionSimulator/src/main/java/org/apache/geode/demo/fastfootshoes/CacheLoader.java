...
public class CacheLoader {
    ...
    private void initalizeBeanReader(String file) {
        ...
        try {
            @SuppressWarnings("unused") final String[] header = beanReader.getHeader(true);
    ...
    private void start() {
        if (currentTransactionCount < current_transactions.size()) {
            ...
            for (int i = 0; i < 25; i++) {
                ...
                Optional<Product> product = productRepository.findById(txn.getProductId());
                Optional<Customer> customer = customerRepository.findById(txn.getCustomerId());
                ...
                if (customer.isPresent() && product.isPresent()
                        && product.get().getStockOnHand() - txn.getQuantity() > 0) {
                    writeTransaction(txn, product.get(), 10.0);
                }
    ...
    private void writeTransaction(Transaction txn, Product product,
        ...
        if (transactionPuts.size() % 10 == 0) {
            transactionRepository.saveAll(transactionPuts);
