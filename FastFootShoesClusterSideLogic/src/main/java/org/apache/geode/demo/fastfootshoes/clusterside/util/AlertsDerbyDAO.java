...
public class AlertsDerbyDAO extends JdbcDaoSupport {
    ...
    public AlertsDerbyDAO(DataSource dataSource) {
        ...
        try {
            createSchema();
        } catch (DataIntegrityViolationException e) { // SQLTransactionRollbackException SQLException
            if (e.getCause() instanceof SQLTransactionRollbackException) {
                SQLTransactionRollbackException e2 = (SQLTransactionRollbackException) e.getCause();
		...
		/* TODO this way of table existence detection does not work.
		try {
