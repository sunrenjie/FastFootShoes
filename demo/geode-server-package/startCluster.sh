#!/bin/bash
# Randy May & Luke Shannon
cd $(dirname $0)
source setenv.sh

LOCATOR_IP=$1

if [ ! -d "$SERVER_DIR_LOCATION/$LOCATOR_1" ]; then
	mkdir $SERVER_DIR_LOCATION/$LOCATOR_1
fi
if [ ! -d "$SERVER_DIR_LOCATION/$LOCATOR_2" ]; then
	mkdir $SERVER_DIR_LOCATION/$LOCATOR_2
fi

echo "Starting Up: Locator A, Locator B and Server 1, Server 2, Server 3 and Server 4 with the FastFoot Shoes Grid Configuration"
echo gfsh start locator --name=$LOCATOR_1 \
  --enable-cluster-configuration=false  \
  --classpath=$CLASSPATH \
  --dir=$SERVER_DIR_LOCATION/$LOCATOR_1 \
  --locators=$LOCATOR_HOST_1[$LOCATOR_PORT_1],$LOCATOR_HOST_2[$LOCATOR_PORT_2] \
  --port=$LOCATOR_PORT_1 \
  --log-level=config \
  --J=-Xms256m \
  --J=-Xmx256m \
  --J=-Dcom.sun.management.jmxremote \
  --J=-Dcom.sun.management.jmxremote.port=15666 \
  --J=-Dcom.sun.management.jmxremote.ssl=false \
  --J=-Dcom.sun.management.jmxremote.authenticate=false \
  --J=-Dcom.sun.management.jmxremote.local.only=false

echo gfsh start locator \
  --name=$LOCATOR_2 \
  --enable-cluster-configuration=false  \
  --classpath=$CLASSPATH \
  --dir=$SERVER_DIR_LOCATION/$LOCATOR_2 \
  --locators=$LOCATOR_HOST_1[$LOCATOR_PORT_1],$LOCATOR_HOST_2[$LOCATOR_PORT_2] \
  --port=$LOCATOR_PORT_2 \
  --log-level=config \
  --J=-Xms256m \
  --J=-Xmx256m \
  --J=-Dcom.sun.management.jmxremote \
  --J=-Dcom.sun.management.jmxremote.port=15667 \
  --J=-Dcom.sun.management.jmxremote.ssl=false \
  --J=-Dcom.sun.management.jmxremote.authenticate=false \
  --J=-Dcom.sun.management.jmxremote.local.only=false

for N in 1 2 3 4 ; do

   NAME="server$N"

   if [ ! -d $SERVER_DIR_LOCATION/$NAME ]; then
	mkdir $SERVER_DIR_LOCATION/$NAME
   fi

   HTTP_PORT=$(( 8180 + $N ))

echo "Starting the server named 'server$N' with config $CONF_DIR/fastfootshoes-spring-cache.xml"

echo gfsh start server \
  --name=$NAME \
  --use-cluster-configuration=false \
  --classpath=$CLASSPATH \
  --server-port=0 \
  --log-level=config \
  --dir=$SERVER_DIR_LOCATION/$NAME \
  --locators=$LOCATOR_HOST_1[$LOCATOR_PORT_1],LOCATOR_HOST_2[$LOCATOR_PORT_2] \
  --J=-Dgemfire.http-service-port=$HTTP_PORT \
  --J=-Dgemfire.start-dev-rest-api=true \
  --J=-Xms256m \
  --J=-Xmx256m \
  --properties-file=$CONF_DIR/gemfire.properties \
  --spring-xml-location=file:///$CONF_DIR/fastfootshoes-spring-cache.xml

done

echo 'Command cheat sheat:'
echo 'gfsh -e connect -e "list members"'
echo 'gfsh -e connect -e "shutdown --include-locators"'
